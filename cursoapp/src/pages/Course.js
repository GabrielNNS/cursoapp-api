...
export default function Course() {
  ...
  useEffect(() => {
    fetch(baseUrl + "/courses/" + id)
      ...
      .then((data) => {
        ...
        fetch(baseUrl + "/professors/" + data.course.professor)
          .then((response) => response.json())
          .then((data) => {
  ...
  useEffect(() => {
    fetch(baseUrl + "/courses/" + id)
      ...
      .then((data) => {
        setCourse(data.course);
        courseVar = data.course;
        fetch(baseUrl + "/professors/" + data.course.professor)
          .then((response) => response.json())
          .then((data) => {
            setQuery(data.professor.name);
          });
      });
  }, []);
        ...
        <div className="max-w-2xl mx-auto mt-4">
            ...
            <div className="relative mt-2 rounded-md shadow-sm">
              <input
                ...
                onChange={(e) => {
                  ...
                  courseVar = { ...courseVar, name: e.target.value };
                }}
              />
            </div>
          </div>
          ...
          <div className="max-w-2xl mx-auto">
            ...
            <textarea
              ...
              onChange={(e) => {
                courseVar = { ...courseVar, description: e.target.value };
              }}
            ></textarea>
          </div>
          ...
          <div className="max-w-2xl mx-auto">
            ...
            <textarea
              ...
              className="block p-2.5 w-full text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
              placeholder="Descrição"
              defaultValue={course.student}
              onChange={(e) => {
                const studentIds = e.target.value
                  .split(",")
                  .map((id) => parseInt(id.trim()));

                courseVar = { ...courseVar, student: studentIds };
              }}
            ></textarea>
          </div>
            ...
            <div className="max-w-2xl mx-auto">
              ...
              <div className="relative mt-2 rounded-md shadow-sm">
                <input
                  ...
                  onChange={(e) => {
                    courseVar = { ...courseVar, classroom: e.target.value };
                  }}
                />
              </div>
            </div>
